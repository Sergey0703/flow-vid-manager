...
- Saves to `blog-drafts/` with frontmatter including ALL fields: `topic_id` (from outline — CRITICAL), `status: draft`, `title`, `date`, `meta_description`, `keywords`, `cover_image: ""`, `sources: []`. **NEVER omit `topic_id` — Editor cannot update DB without it.**
