...
logger = logging.getLogger("aimediaflow-secretary")
...
class SecretaryAgent(Agent):
    ...
    async def on_user_turn_completed(self, turn_ctx, new_message):
        ...
        # Detect farewell — schedule room deletion after reply
...
async def entrypoint(ctx: JobContext):
    ...
    async def send_report():
        logger.info("Secretary session ended, sending report...")
        await session_log.send_email()

    ctx.add_shutdown_callback(send_report)
    await ctx.connect()
    logger.info("Secretary agent connected to LiveKit room")
